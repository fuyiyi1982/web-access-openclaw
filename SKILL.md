...
name: web-access
license: MIT
github: https://github.com/fuyiyi1982/web-access-openclaw
description: 当需要通过 CDP 浏览器执行网页抓取、登录后操作、网络交互、抓取社交媒体内容（小红书、微博、推特等）、读取动态渲染页面、以及任何需要真实浏览器环境的网络任务时，必须通过此 skill 处理。
metadata: { "openclaw": { "emoji": "🌐", "homepage": "https://github.com/eze-is/web-access", "requires": { "bins": ["node", "curl"] } }, "author": "一泽Eze", "version": "2.3.0" }
---
...
| 场景 | 工具 |
|------|------|
| 搜索摘要或关键词结果，发现信息来源 | **Brave Search**（内置插件，直接使用） |
| URL 已知，需要快速提取页面正文内容（省 token） | **Jina**（`bash` 执行 `curl -s r.jina.ai/example.com`） |
| URL 已知，需要原始 HTML 源码（meta、JSON-LD 等结构化字段） | **curl**（`bash` 执行 `curl -s URL`） |
| 非公开内容，或已知静态层无效的平台（小红书、微信公众号等公开内容也被反爬限制） | **浏览器 CDP**（直接，跳过静态层） |
| 需要登录态、交互操作，或需要像人一样在浏览器内自由导航探索 | **浏览器 CDP** |

浏览器 CDP 不要求 URL 已知——可从任意入口出发，通过页面内搜索、点击、跳转等方式找到目标内容。Brave Search、Jina、curl 均不处理登录态。
...
任务包含多个**独立**调研目标时（如同时调研 N 个项目、N 个来源），鼓励通过 `sessions_spawn` 创建子 session 并行执行，而非主 Agent 串行处理。

**好处：**
- **速度**：多子 session 并行，总耗时约等于单个子任务时长
- **上下文保护**：抓取内容不进入主 Agent 上下文，主 Agent 只接收摘要，节省 token

**并行 CDP 操作**：每个子 session 在当前用户浏览器实例中，自行创建所需的后台 tab（`/new`），自行操作，任务结束自行关闭（`/close`）。所有子 session 共享一个 Chrome、一个 Proxy，通过不同 targetId 操作不同 tab，无竞态风险。

**子 Session Prompt 写法：目标导向，而非步骤指令**
- 通过 `sessions_spawn` 创建子 session 时，在消息中写明 `必须加载 web-access skill 并遵循指引`，子 session 会自动加载 skill。
- 子 session 有自主判断能力。主 Agent 的职责是说清楚**要什么**，仅在必要与确信时限定**怎么做**。过度指定步骤会剥夺子 session 的判断空间，反而引入主 Agent 的假设错误。**避免 prompt 用词对子 session 行为的暗示**：「搜索xx」会把子 session 锚定到 Brave Search，而实际上有些反爬站点需要 CDP 直接访问主站才能有效获取内容。主 Agent 写 prompt 时应描述目标（「获取」「调研」「了解」），避免用暗示具体手段的动词（「搜索」「抓取」「爬取」）。
- 可通过 `sessions_send` 向子 session 发送后续指令，或通过 `sessions_history` 获取子 session 的执行结果。
...
| 需要 CDP 浏览器或长时间运行的任务 | 几次 Brave Search / Jina 就能完成的轻量查询 |
...
操作中积累的特定网站经验，按域名存储在 `{baseDir}/references/site-patterns/` 下。

已有经验的站点：!`ls {baseDir}/references/site-patterns/ 2>/dev/null | sed 's/\.md$//' || echo "暂无"`
